...
export function NewOrderForm() {
  ...
  return (
    <form className="panel" onSubmit={confirmAll}>
      <div className="panel-head"><h2>New Multi-Parcel Order</h2><span className="badge">Step {step}/3</span></div>

      {step === 1 ? (
        <>
          <label>Select Shipment Type</label>
          <select value={routeType} onChange={(e) => setRouteType(e.target.value as "EUROPE_TO_EUROPE" | "US" | "") }>
      ...
      {step === 2 ? (
        <>
          <label>Receiver Name</label><input value={parcelForm.title} onChange={(e) => setParcelForm((p) => ({ ...p, title: e.target.value }))} />
          <label>Address Details</label><textarea value={parcelForm.address} onChange={(e) => setParcelForm((p) => ({ ...p, address: e.target.value }))} />
          <label>City</label><input value={parcelForm.city} onChange={(e) => setParcelForm((p) => ({ ...p, city: e.target.value }))} />
          <label>State</label><input value={parcelForm.state} onChange={(e) => setParcelForm((p) => ({ ...p, state: e.target.value }))} />
          <label>Zip Code</label><input value={parcelForm.zipCode} onChange={(e) => setParcelForm((p) => ({ ...p, zipCode: e.target.value }))} />
          <label>Country</label><input value={parcelForm.country} onChange={(e) => setParcelForm((p) => ({ ...p, country: e.target.value }))} />
          <label>Contact Phone</label><input value={parcelForm.contactPhone} onChange={(e) => setParcelForm((p) => ({ ...p, contactPhone: e.target.value }))} />

          <label className="choice"><input type="checkbox" checked={parcelForm.saveAddress} onChange={(e) => setParcelForm((p) => ({ ...p, saveAddress: e.target.checked }))} />Save this address in Recipients</label>

          <h3>Products and Quantity</h3>
          <div className="item-row">
            <input value={productName} onChange={(e) => setProductName(e.target.value)} placeholder="Product name" />
            <input type="number" min={1} value={productQty} onChange={(e) => setProductQty(Number(e.target.value) || 1)} />
          </div>
          <button type="button" className="secondary" onClick={addProductToParcel}>+ Add Product</button>
...
>
  <Trash2 size={16} />
          ...
          <button type="button" onClick={addOrUpdateParcel}>{editingIndex !== null ? "Update Parcel" : "Add Parcel"}</button>
        </>
      ) : null}

      {step === 3 ? (
        <>
          <div className="panel-head"><h3>Review All Parcels</h3><span className="badge">{parcels.length} parcels</span></div>
          {parcels.map((p, idx) => (
            <div className="order-card" key={idx}>
              ...
              <p>{p.address}</p>
              <p>{p.city}, {p.state} - {p.zipCode}, {p.country}</p>
              <p>Save Address: {p.saveAddress ? "Yes" : "No"}</p>
              <ul>{p.items.map((i, x) => <li key={x}>{i.name} x {i.quantity}</li>)}</ul>
            </div>
          ))}
        </>
      ) : null}
      ...
      <div className="wizard-actions">
        <button type="button" className="secondary" onClick={() => setStep((s) => Math.max(1, s - 1))} disabled={step === 1}>Back</button>
        {step < 3 ? (
